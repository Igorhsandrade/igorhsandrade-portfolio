...
import { Code, Cpu, Database, Wrench, Calendar, Search, X } from "lucide-react"
...
export function SkillsSection({ skills }: SkillsSectionProps) {
  const skillsByCategory = useMemo(() => {
    ...
    Object.keys(grouped).forEach((category) => {
      grouped[category].sort((a, b) => levelOrder[b.level] - levelOrder[a.level])
    })
  ...
  const totalFilteredSkills = Object.values(filteredSkillsByCategory).flat().length
  ...
  useEffect(() => {
    if (searchTerm.trim()) {
      ...
      if (categoriesWithResults.length > 0) {
        ...
        if (!filteredSkillsByCategory[activeTab] || filteredSkillsByCategory[activeTab].length === 0) {
    ...
    return (
      <div className="flex items-center justify-between p-4 rounded-lg border hover:bg-muted/50 transition-colors group">
        <div className="flex items-center gap-3">
          <IconComponent className={cn("w-5 h-5", getLevelColor(skill.level))} />
          <div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="w-32">
            ...
            <div className="text-xs text-center mt-1 text-muted-foreground">{skill.level}</div>
          </div>
        </div>
      </div>
    )
    ...
    <div className="space-y-6">
      ...
      {searchTerm && (
        <div className="mb-4 text-sm text-muted-foreground">
          Found {totalFilteredSkills} skill{totalFilteredSkills !== 1 ? "s" : ""} matching "{searchTerm}"
          {totalFilteredSkills > 0 && (
            <span className="ml-2 text-teal-600">
              • Showing results in {Object.keys(filteredSkillsByCategory).length} categor
            ...
            <TabsContent key={category} value={category} className="mt-6">
              <div className="space-y-3">
                {categorySkills.length > 0 ? (
                  categorySkills.map((skill) => <SkillListItem key={skill.name} skill={skill} />)
                ) : (
                  <div className="text-center py-8 text-muted-foreground">
                    {searchTerm ? (
                      <>
                        ...
                        <div className="text-sm">Try a different search term or browse other categories</div>
                      </>
                    ) : (
                      <>
                        <div className="mb-2">No skills in this category yet</div>
                        <div className="text-sm">Skills will be added as I learn new technologies</div>
                      </>
                    )}
                  </div>
                )}
